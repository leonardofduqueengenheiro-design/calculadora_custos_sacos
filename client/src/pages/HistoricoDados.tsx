...
function percentualVariacao(inicial: number, seguinte: number) {
  if (Math.abs(inicial) < 0.005) return null;
  return ((seguinte - inicial) / Math.abs(inicial)) * 100;
}

function Delta({ value, percentual, aumentoFavoravel = false }: { value: number; percentual?: number | null; aumentoFavoravel?: boolean }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-xs font-semibold tabular-nums" style={{ color }}>
      ...
      <span>{value > 0 ? "+" : ""}{formatBRL(value, 2)}</span>
      {percentual !== null && percentual !== undefined && <span className="opacity-80">({percentual > 0 ? "+" : ""}{percentual.toLocaleString("pt-BR", { maximumFractionDigits: 1 })}%)</span>}
    </span>
  );
}
...
export default function HistoricoDados() {
  ...
  const maioresReducoes = useMemo(() => linhasComparacao
    .map(linha => ({ ...linha, variacao: linha.comparada - linha.referencia }))
    .filter(linha => linha.variacao < -0.005)
    .sort((a, b) => a.variacao - b.variacao)
    .slice(0, 3), [linhasComparacao]);

  const maioresAumentos = useMemo(() => linhasComparacao
    .map(linha => ({ ...linha, variacao: linha.comparada - linha.referencia }))
    .filter(linha => linha.variacao > 0.005)

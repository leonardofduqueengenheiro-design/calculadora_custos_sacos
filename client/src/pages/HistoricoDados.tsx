...
import {
  Archive, CalendarRange, Database, FileSpreadsheet, RefreshCw,
  RotateCcw, ArrowRightLeft, TrendingDown, TrendingUp,
} from "lucide-react";
...
function parseMedias(value: string): Record<string, number> {
  try {
    ...
    return Object.fromEntries(Object.entries(parsed).filter(([, media]) => typeof media === "number" && Number.isFinite(media)) as Array<[string, number]>);
  } catch {
    return {};
  }
}
...
function Delta({ value }: { value: number }) {
  ...
  return <span className="inline-flex items-center gap-1 text-xs font-semibold tabular-nums" style={{ color }}>{positive ? <TrendingUp className="h-3.5 w-3.5" /> : negative ? <TrendingDown className="h-3.5 w-3.5" /> : null}{value > 0 ? "+" : ""}{formatBRL(value, 2)}</span>;
}
...
export default function HistoricoDados() {
  ...
  useEffect(() => {
    ...
    if (baseId === null || !importacoes.some(item => item.id === baseId)) setBaseId(ativaId);
    if (compararId === null || !importacoes.some(item => item.id === compararId) || compararId === baseId) {
      const alternativa = importacoes.find(item => item.id !== baseId);
      setCompararId(alternativa?.id ?? null);
    }
  }, [importacoes, ativa?.id, baseId, compararId]);
  ...
  const categoriasComparadas = useMemo(() => Array.from(new Set([...Object.keys(mediasBase), ...Object.keys(mediasComparacao)])).sort(), [mediasBase, mediasComparacao]);
        ...
        <>
              ...
              <>
                ...
                {base && comparacao && (
                  <div className="mt-5 overflow-hidden rounded-lg" style={{ border: "1px solid var(--border)" }}>
                    <div className="overflow-x-auto"><table className="w-full min-w-[680px]"><thead><tr style={{ background: "var(--muted)", borderBottom: "1px solid var(--border)" }}><th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted-foreground)" }}>Categoria</th><th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted-foreground)" }}>Referência</th><th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted-foreground)" }}>Comparada</th><th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted-foreground)" }}>Variação</th></tr></thead><tbody className="divide-y" style={{ borderColor: "var(--border)" }}>
                      {[{ key: "materia_prima", label: "Matéria-Prima", base: parseFloat(base.mediaMateriaPrima), comparada: parseFloat(comparacao.mediaMateriaPrima) }, ...categoriasComparadas.map(categoria => ({ key: categoria, label: CATEGORIA_LABELS[categoria] ?? categoria, base: mediasBase[categoria] ?? 0, comparada: mediasComparacao[categoria] ?? 0 }))].map(linha => <tr key={linha.key} style={{ background: "var(--card)" }}><td className="px-4 py-3 text-sm" style={{ color: "var(--foreground)" }}>{linha.label}</td><td className="px-4 py-3 text-right text-sm tabular-nums" style={{ color: "var(--muted-foreground)" }}>{formatBRL(linha.base, 2)}/mês</td><td className="px-4 py-3 text-right text-sm font-medium tabular-nums" style={{ color: "var(--foreground)" }}>{formatBRL(linha.comparada, 2)}/mês</td><td className="px-4 py-3 text-right"><Delta value={linha.comparada - linha.base} /></td></tr>)}
                    </tbody></table></div>
                    <div className="grid grid-cols-1 gap-px border-t sm:grid-cols-3" style={{ background: "var(--border)", borderColor: "var(--border)" }}>{[
                      { label: "Total de custos", base: parseFloat(base.totalCustos), comparada: parseFloat(comparacao.totalCustos) },
                      { label: "Matéria-prima no período", base: parseFloat(base.totalMateriaPrima), comparada: parseFloat(comparacao.totalMateriaPrima) },
                      { label: "Faturamento identificado", base: parseFloat(base.totalFaturamento), comparada: parseFloat(comparacao.totalFaturamento) },
                    ].map(item => <div key={item.label} className="p-3" style={{ background: "var(--card)" }}><p className="text-xs" style={{ color: "var(--muted-foreground)" }}>{item.label}</p><div className="mt-1 flex items-center justify-between gap-2"><span className="text-sm font-semibold tabular-nums" style={{ color: "var(--foreground)" }}>{formatBRL(item.comparada, 0)}</span><Delta value={item.comparada - item.base} /></div></div>)}</div>
                  </div>
                )}
              </>
            )}
          ...
          <div className="space-y-4">
            {importacoes.map(importacao => {
              ...
              return <article key={importacao.id} className="overflow-hidden rounded-xl card-gradient" style={{ border: `1px solid ${ehAtiva ? "oklch(0.70 0.18 155 / 0.38)" : "var(--border)"}` }}>
